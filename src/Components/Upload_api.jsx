...
const Upload_api = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    await Promise.all(
      files.map(async (file, index) => {
        ...
        formDataFile.append("music", file);
        try {
          const response = await UrlAxios.post("convertir", formDataFile, {
            headers: {
            ...
            onUploadProgress: (progressEvent) => {
              // Puedes manejar aquí el progreso de la carga del archivo
        ...
        setFilesMusic((prevFilesMusic) =>
          prevFilesMusic.concat(compressedFile)
        );
      })
    );
  ...
  //               ((index + 1) / files.length) * 100 + progress / files.length
  //             );
  //             console.log(`Progreso del archivo ${index + 1}: ${progress}%`);
  //           },
  //         });
  ...
  //   // Establece el progreso en 100% al finalizar la carga de todos los archivos
  //   setUploadProgress(100);
  //   toastify("Archivos convertidos", true);
  // };
  ...
  const downloadZip = async () => {
    const zip = new JSZip();
    console.log(filesMusic);
    if (filesMusic.length === 0) return toastify("No hay archivos", false);
    // Itera sobre cada archivo en filesImages
    for (let i = 0; i < filesMusic.length; i++) {
      const file = filesMusic[i];
      let blobData;
      ...
      zip.file(`${file.name.split(".")[0]}.mp3`, blobData); // Asegúrate de usar la extensión de archivo correcta
    }
